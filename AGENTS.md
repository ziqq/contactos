...
This is a **Flutter plugin** (not an app) for accessing and managing the device's contacts. It follows the [federated plugin architecture](https://flutter.dev/go/federated-plugins) with platform-specific implementations for Android and iOS, as well as a common Dart platform interface.
...
Each package (`contactos/`, `contactos_platform_interface/`, `contactos_android/`, `contactos_foundation/`) has its own `Makefile` with the same targets. Run from within the package directory:
...
- **Method channels**: use `MethodChannelContactos` for platform communication.
