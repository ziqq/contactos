...
class _ContactsListPageState extends State<ContactsListScreen> {
  ...
  Future<void> refreshContacts() async {
    // final contacts =
    //   await ContactosPluginAndroid
    //   .instance
    //   .getContactsForPhone("8554964652");

    // Load without thumbnails initially.
    final contacts = await ContactosPluginAndroid.instance.getContacts(
    ...
    for (var contact in _contacts ?? contacts) {
      ContactosPluginAndroid.instance.getAvatar(contact).then((avatar) {
  ...
  Future<void> _openContactForm() async {
    try {
      var _ = await ContactosPluginAndroid.instance.openContactForm(
        androidLocalizedLabels: kAndroidLocalizedLabels,
      );
      await refreshContacts();
...
/// State for widget [_ContactDetailsPage].
class _ContactDetailsPageState extends State<_ContactDetailsPage> {
  Future<void> _openExistingContactOnDevice(BuildContext context) async {
    try {
      final contact = await ContactosPluginAndroid.instance.openExistingContact(
        widget._contact,
        androidLocalizedLabels: kAndroidLocalizedLabels,
      );
    ...
    appBar: AppBar(
      ...
      actions: <Widget>[
        ...
        IconButton(
          ...
          onPressed: () {
            ContactosPluginAndroid.instance.deleteContact(widget._contact);
          },
        ),
        IconButton(
...
/// Tile for contact items like phones, emails etc.
/// {@macro add_contact_screen}
      ...
      actions: <Widget>[
        TextButton(
          onPressed: () {
            ...
            ContactosPluginAndroid.instance.addContact(newContact);
            Navigator.of(context).pop();
          },
      ...
      actions: <Widget>[
        IconButton(
          ...
          onPressed: () async {
            ...
            _formKey.currentState?.save();
            await ContactosPluginAndroid.instance.updateContact(newContact);
            navigator
                .pushReplacement(
                  MaterialPageRoute<void>(
