...
@available(iOS 9.0, *)
public class ContactosPlugin: NSObject, FlutterPlugin, CNContactViewControllerDelegate, CNContactPickerDelegate {
         ...
         do {
            // Check to make sure dictionary has an identifier
             guard let identifier = contact["identifier"] as? String else{
                 result(ContactosPlugin.FORM_COULD_NOT_BE_OPEN)
                 return nil;
             }
            ...
            DispatchQueue.main.async {
                ...
                if let viewFrame = currentViewController?.view.frame {
                    activityIndicatorView.frame = viewFrame
                }
